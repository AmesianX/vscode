...
export class MarkdownContentProvider {
	...
	public async provideTextDocumentContent(
		...
		const initialData = {
			...
			webviewResourceRoot: (await resourceProvider.toWebviewResource(markdownDocument.uri)).toString(),
		};
		...
		const csp = await this.getCsp(resourceProvider, sourceUri, nonce);
		...
		return `<!DOCTYPE html>
			...
			<head>
				...
				<script src="${await this.extensionResourcePath(resourceProvider, 'pre.js')}" nonce="${nonce}"></script>
				${await this.getStyles(resourceProvider, sourceUri, nonce, config, state)}
				<base href="${await resourceProvider.toWebviewResource(markdownDocument.uri)}">
			</head>
			<body class="vscode-body ${config.scrollBeyondLastLine ? 'scrollBeyondLastLine' : ''} ${config.wordWrap ? 'wordWrap' : ''} ${config.markEditorSelection ? 'showEditorSelection' : ''}">
				${body}
				<div class="code-line" data-line="${markdownDocument.lineCount}"></div>
				${await this.getScripts(resourceProvider, nonce)}
			</body>
			</html>`;
	}
	...
	private async extensionResourcePath(resourceProvider: WebviewResourceProvider, mediaFile: string): Promise<string> {
		const webviewResource = await resourceProvider.toWebviewResource(
			vscode.Uri.file(this.context.asAbsolutePath(path.join('media', mediaFile))));
		return webviewResource.toString();
	}

	private async fixHref(resourceProvider: WebviewResourceProvider, resource: vscode.Uri, href: string): Promise<string> {
		if (!href) {
			return href;
		}
		...
		if (path.isAbsolute(href)) {
			return (await resourceProvider.toWebviewResource(vscode.Uri.file(href))).toString();
		}

		// Use a workspace relative path if there is a workspace
		const root = vscode.workspace.getWorkspaceFolder(resource);
		if (root) {
			return (await resourceProvider.toWebviewResource(vscode.Uri.file(path.join(root.uri.fsPath, href)))).toString();
		}

		// Otherwise look relative to the markdown file
		return (await resourceProvider.toWebviewResource(vscode.Uri.file(path.join(path.dirname(resource.fsPath), href)))).toString();
	}

	private async computeCustomStyleSheetIncludes(resourceProvider: WebviewResourceProvider, resource: vscode.Uri, config: MarkdownPreviewConfiguration): Promise<string> {
		if (!Array.isArray(config.styles)) {
			return '';
		}
		const out: string[] = [];
		for (const style of config.styles) {
			out.push(`<link rel="stylesheet" class="code-user-style" data-source="${escapeAttribute(style)}" href="${escapeAttribute(await this.fixHref(resourceProvider, resource, style))}" type="text/css" media="screen">`);
		}
		return out.join('\n');
	}
	...
	private async getStyles(resourceProvider: WebviewResourceProvider, resource: vscode.Uri, nonce: string, config: MarkdownPreviewConfiguration, state?: any): Promise<string> {
		const baseStyles: string[] = [];
		for (const resource of this.contributionProvider.contributions.previewStyles) {
			baseStyles.push(`<link rel="stylesheet" type="text/css" href="${escapeAttribute(await resourceProvider.toWebviewResource(resource))}">`);
		}

		return `${baseStyles.join('\n')}
			${this.getSettingsOverrideStyles(nonce, config)}
			${await this.computeCustomStyleSheetIncludes(resourceProvider, resource, config)}
			${this.getImageStabilizerStyles(state)}`;
	}

	private async getScripts(resourceProvider: WebviewResourceProvider, nonce: string): Promise<string> {
		const out: string[] = [];
		for (const resource of this.contributionProvider.contributions.previewScripts) {
			out.push(`<script async
				src="${escapeAttribute(await resourceProvider.toWebviewResource(resource))}"
				nonce="${nonce}"
	...
	private async getCsp(
		provider: WebviewResourceProvider,
		resource: vscode.Uri,
		nonce: string
	): Promise<string> {
		const rule = await provider.cspRule;
